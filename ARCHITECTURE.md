...
剩余百分比和重置时间。`RemainingPercent` / `HasProgress` 对 Codex 只代表 5 小时
窗口：5 小时窗口缺失时必须为 `0 / false`，即使每周窗口仍有效也不得回退替代。
贴边能量条、趋势、低额度提醒和快速下降提醒均消费这一 5 小时主指标；每周窗口
只作为详情页的并列周期信息展示。
...
- 完整状态保存在 `%LOCALAPPDATA%\RemainingMarginFloat\state-history`，使用
  ...
  序列化前排除。
- 全量状态严格使用滚动 168 小时保留期；启动和每次保存都重写原子索引、清理
  过期节点及未引用对象。读取最新对象失败时必须向前回退到最近的有效对象。
...
- Codex 官方与本地限额都必须按窗口时长识别 300 分钟的 5 小时窗口和
  10080 分钟的每周窗口，不得假定 `primary` / `secondary` 的固定含义。旧状态若
  没有明确的 5 小时字段，只能展示为 5 小时未知，不能用旧的每周主值驱动提醒或趋势。
...
- 趋势历史 v3 同时保存 UTC、当前本地日期、时区、偏移和额度周期；读取、导入时
  按当前时区重新校准本地日期，并兼容 DeepSeek 的 v1/v2 JSONL。Codex 旧记录
  未区分每周与 5 小时窗口，必须排除无 `FiveHour` 周期标识的旧百分比样本，避免
