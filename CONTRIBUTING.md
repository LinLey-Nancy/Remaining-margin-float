...
感谢你改进 Codex Margin Float。提交前请保持免安装、Windows 原生、数据最小化和密钥不明文这几个产品约束。
...
- 可选：Claude Code DeepSeek 会话数据，用于验证本地 Token 汇总
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\src\CodexMarginFloat.ps1 -CheckDeepSeekData

powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\src\CodexMarginFloat.ps1 -CheckDeepSeekUsage
```
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -STA `
  -File .\src\CodexMarginFloat.ps1 `
  -Demo -DemoProvider deepseek -RenderPreview expanded `
  -PreviewPath .\preview-deepseek-expanded.png
```
...
- 网络端点必须固定为官方 DeepSeek 域名，不允许把 API Key 发送给自定义地址。
- API Key 只能来自环境变量或 DPAPI 密文，不能进入测试输出、日志或 Git。
- 新的可见交互需要键盘入口或自动化名称。
- 不添加网络遥测；余额查询不是遥测，且只在 DeepSeek Provider 激活时运行。
