...
- 80×80 正方形紧凑悬浮窗：Codex 显示 5 小时余量，DeepSeek 显示余额或预算百分比
- 可选贴边隐藏：拖到屏幕左侧或右侧后自动吸附，只保留贴边的 14px 竖向能量条触发区
- 贴边后悬停会以滑动动画展示紧凑布局，移开后自动滑回超紧凑布局
- 常规界面使用低饱和连续色阶；Codex 贴边能量条严格跟随 5 小时余量，缺失时显示灰色未知状态，不用每周额度替代
- 双色进度条：鼠尾草绿表示剩余，暖米灰表示已使用
- 详情进度条上方右对齐显示下次重置日期与倒计时
- Codex 详情使用两条周期带同时展示 5 小时与每周的已用量、余额和各自重置时间；今日 Token、输出、缓存和命中率保留在次级摘要
...
- 官方接口在后台异步刷新，不会阻塞悬浮窗；5 小时窗口缺失时明确显示“5 小时余量未知”，即使每周窗口仍然可用也不会冒充 5 小时数据
...
- 本地记录脱敏余量样本；Codex 的趋势、低额度提醒和快速下降提醒严格跟随 5 小时额度，在详情中显示带真实时间轴、起止值和样本数的 24 小时与近 7 日趋势
...
会依据目标电脑时区重新校准日期。Codex 1.9.0 起只把明确标识为 5 小时额度的
样本用于趋势和提醒；旧版本留下的无周期标识 Codex 百分比样本不会混入新趋势。
