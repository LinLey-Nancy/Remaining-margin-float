...
安全修复面向最新 GitHub Release。旧版本不单独维护。
...
- Windows 安装程序及其 SHA-256；
- 与版本标签一致的源代码；
- 自动诊断、真实安装/卸载和发布策略检查结果。

当前发布物不进行 Authenticode 代码签名，Windows 会显示“未知发布者”。
请只从上述官方 Release 页面下载，并使用同名 `.sha256` 文件核对完整性。
应用内更新还会限制下载地址、版本和资产命名必须对应本仓库正式 Release，
并自动核对文件大小、SHA-256 与安装程序内嵌版本。手动更新不会自动执行
未签名安装程序，只会按用户选择打开文件所在位置。安装版的自动更新默认关闭；
只有用户明确启用并接受未签名风险警告后，应用才会在 Windows 判定为合适的
网络上静默运行已校验的安装程序。自动更新不传入新安装目录、不强制结束应用，
也不允许安装器重启 Windows。
SHA-256 只能帮助确认下载完整性，不能独立证明发布者身份，也不能替代
源代码审查或安全软件检测。
